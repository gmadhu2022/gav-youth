...
async def get_conversation(user_id: str, conversation_id: str, limit: int = 30):
    ...
    return {"other": other, "messages": msgs, "has_more": len(msgs) == limit}


async def get_messages_before(user_id: str, conversation_id: str, before: str, limit: int = 30):
    await _require_participant(conversation_id, user_id)
    msgs, _ = await rest_get(
        "messages",
        {"conversation_id": f"eq.{conversation_id}", "created_at": f"lt.{before}",
         "order": "created_at.desc", "limit": str(limit), "select": "*"},
    )
    msgs = list(reversed(msgs))
...
# ---------------- messages ----------------
MEDIA_TYPES = {"image", "file", "audio"}
MEDIA_LABEL = {"image": "📷 Photo", "file": "📎 File", "audio": "🎙️ Voice note"}


async def send_message(user_id: str, conversation_id: str, data: dict):
    ...
    if msg_type == "text":
        ...
        if not data.get("attachment_path"):
            raise HTTPException(status_code=400, detail="Attachment is missing")
        content = content or MEDIA_LABEL[msg_type]   # preview text for the chat list
    else:
    ...
    for k in ("attachment_path", "attachment_name", "attachment_size", "attachment_mime", "duration_ms"):
        if data.get(k) is not None:
            row[k] = data[k]

    # Hook point for moderation / notifications before the message lands.
    rows = await rest_insert("messages", row)
    msg = rows[0]
    # fire-and-forget push to the other participant(s)
    try:
        asyncio.create_task(notifications.notify_new_message(user_id, conversation_id, content))
    except Exception:
        pass
    return msg
