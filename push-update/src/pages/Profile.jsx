...
export default function Profile() {
  ...
  const togglePush = async () => {
    ...
    try {
      if (pushOn) { await disablePush(); setPushOn(false); setPushMsg('Notifications turned off.') }
      else { await enablePush(); setPushOn(true); setPushMsg('Notifications are on for this device.') }
    } catch (e) {
      setPushMsg(e.message || 'Could not change notifications.')
      ...
      <div className="profile-body">
        ...
        {pushSupported() && (
          <button className="btn-ghost" onClick={togglePush} disabled={pushBusy}>
            {pushBusy ? <Loader2 size={18} className="spin" /> : <Bell size={18} />}
            {pushOn ? 'Turn off notifications' : 'Enable notifications'}
          </button>
        )}
        {pushMsg && <div className="privacy-note" style={{ marginTop: 0 }}><span>{pushMsg}</span></div>}
